...
class GapStatistics:
    ...
    def _calculate_Wks(self, K: int, X: np.ndarray) -> list:
        ...
        for k in np.arange(1, K+1):
            try:
                model = self.algorithm(n_clusters=k, n_init=10).fit(X)
    ...
    def _simulate_Wks(self, X: np.ndarray, K: int, n_iterations: int) -> np.ndarray:
        ...
        for _ in range(n_iterations):
            Z_prime = np.random.uniform(
                np.min(X_prime), np.max(X_prime), size=(len(X), X.shape[1]))
    ...
    def fit_predict(self, K: int, X: np.ndarray, n_iterations: int = 20):
        ...
        gaps = np.mean(log_Wks_star, axis=0) - log_Wks
        ...
        self.params = {'Wks': Wks, 'sim_Wks': sim_Wks,
                       'sim_sks': sim_sks, 'gaps': gaps}
    ...
    def plot(self, original_labels: list = None, colors: dict = None):
        ...
        algorithm = self.algorithm(n_clusters=self.optimum, n_init=10).fit(self.X)
        ...
        if original_labels:
            for i, x in enumerate(self.X):
                plt.scatter(x[0], x[1], color=mapped_colors[i])
        ...
        plt.plot(log_sim_Wks, '-o', color="green",
                 label=f"Logged Simulated Wks [{self.n_iterations}]")
        ...
        yminx = (((gaps - sim_sks) - np.min(gaps)) /
                 (np.max(gaps) - np.min(gaps)))
        ymaxsx = (((gaps + sim_sks) - np.min(gaps)) /
                  (np.max(gaps) - np.min(gaps)))
